...
2022-10-28 Update AG: Moved the tabulation of steroids for more than two days/allocation to the top, as the dataset isn't loaded anymore in the place the code was previously inserted. No other changes.
...
We already have these from Tables 1 and 2 of the primary paper, just need to add a summary for one more covariate.

```{r}
table(dat$corticosteroids_before_more_than_2days, dat$allocation)
```
...
Next we create some additional figures and tables for the paper. W
...
read_docx() %>% 
  body_add_par("Treatment Effects by Quartile Table") %>% 
  body_add_flextable(value = ft) %>% 
  print(target = "treatment_effect_table.docx")
```
...
weight_list <- seq(-1.5, 3.5, length.out = 10)
...
figdat1_age$covar_val <- figdat1_age$covar_val * age_sd + age_mean
figdat2_age$covar_val <- figdat2_age$covar_val * age_sd + age_mean
figdat1_weight$covar_val <- figdat1_weight$covar_val * weight_sd + weight_mean
figdat2_weight$covar_val <- figdat2_weight$covar_val * weight_sd + weight_mean
...
fig2 <- ggplot(figdat_dep, aes(x = covar_val, y = cate)) +
  ...
  geom_smooth(aes(ymin = cate_lower, ymax = cate_upper), stat = "identity",
              colour = "black") +
  xlab("Top row: Age in years            Bottom row: Weight in kg") + ylab("Average CATE") +
  theme_bw() +
  scale_fill_grey()

# Output as PDF
pdf(file = "newfig2.pdf")
fig2
dev.off()
```
