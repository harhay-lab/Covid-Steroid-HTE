...
for (i in 1:length(age_list)) {
  ...
  figdat1_age$cate_lower[i] <-
    mean(apply(exp(bartmod1$yhat.test.collapse) /
            (1 + exp(bartmod1$yhat.test.collapse)) -
          exp(bartmod0$yhat.test.collapse) /
            (1 + exp(bartmod0$yhat.test.collapse)),
          2, quantile, probs = 0.025))
  ...
  figdat2_age$cate_lower[i] <-
    mean(apply(bartmod1_c$yhat.test.collapse - bartmod0_c$yhat.test.collapse,
          2, quantile, probs = 0.025))
...
fig2 <- ggplot(figdat_dep, aes(x = covar_val, y = cate)) +
  facet_grid(covar_type~outcome, scales = "free") +
  geom_smooth(aes(ymin = cate_lower, ymax = cate_upper), stat = "identity",
              colour = "black") +
