...
set.seed(60622)
bartmod1 <- mc.lbart(x.train = dat[, c(1, 4:13)], y.train = dat$dead90,
                     x.test = dat1[, c(1, 4:13)], mc.cores = 4, ndpost = 4000)
bartmod0 <- mc.lbart(x.train = dat[, c(1, 4:13)], y.train = dat$dead90,
                     x.test = dat0[, c(1, 4:13)], mc.cores = 4, ndpost = 4000)
```
...
# Fit BART models under default hyperparameters, get predictions under each trt
set.seed(60622)
bartmod1_c <- mc.wbart(x.train = dat[, c(1, 4:13)], y.train = dat$dawols90,
                       x.test = dat1[, c(1, 4:13)], mc.cores = 4,
                       ndpost = 4000)
bartmod0_c <- mc.wbart(x.train = dat[, c(1, 4:13)], y.train = dat$dawols90,
                       x.test = dat0[, c(1, 4:13)], mc.cores = 4,
                       ndpost = 4000)
...
par(mfrow = c(2, 2))
plot(bartmod0_c$sigma[, 1], type = "l")
abline(v = 100, lwd = 2, col = "red")
plot(bartmod0_c$sigma[, 2], type = "l")
abline(v = 100, lwd = 2, col = "red")
plot(bartmod0_c$sigma[, 3], type = "l")
abline(v = 100, lwd = 2, col = "red")
plot(bartmod0_c$sigma[, 4], type = "l")
abline(v = 100, lwd = 2, col = "red")
...
One should check that each chain has converged after burn-in (designated by the vertical red lines).
...
```{r}
# First for bartmod0
auto.corr <- acf(bartmod0$yhat.train[ , sample(1:dim(dat)[1], 10)], plot=FALSE)
...
# Then for bartmod1
auto.corr <- acf(bartmod1$yhat.train[ , sample(1:dim(dat)[1], 10)], plot=FALSE)
max.lag <- max(auto.corr$lag[ , 1, 1])
...
```{r}
# First for bartmod0
geweke <- gewekediag(bartmod0$yhat.train)
n <- dim(dat)[1]
j <- -10^(log10(n) - 1)
...
# Then for bartmod1
geweke <- gewekediag(bartmod1$yhat.train)
plot(geweke$z, pch = '.', cex = 2, ylab = 'z', xlab = 'i',
     xlim=c(j, n), ylim=c(-5, 5))
lines(1:n, rep(-1.96, n), type='l', col=6)
lines(1:n, rep(+1.96, n), type='l', col=6)
